<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:76:"D:\phpStudy\PHPTutorial\WWW\resource/application/index\view\index\video.html";i:1541743266;s:81:"D:\phpStudy\PHPTutorial\WWW\resource\application\index\view\common\userlogin.html";i:1540259706;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<title>课程</title>
...
setInterval('time1()',5000);//上报视频的观看时间
setInterval('time()',5000);//兼容cookie.js中方法，不能再使用windows.onload
setTimeout('videolist()',10000);//上报视频的浏览记录
setTimeout('videoclick()',5000);//上报视频的点击率
//上报视频点击率
function videoclick(){
	url=getRootPath()+"/index.php/Index/index/clickvideo";
	if(window.localStorage){
		var video_id="<?php echo $video['videotab_id']; ?>";
		var video_type="<?php echo $video['video_type']; ?>";
		var key=video_id+'-'+video_type;
		cleanStorage();
		var click=localStorage.getItem(key);
		if(!click){
			$.ajax({
				url:url,
...
function cleanStorage(){
	...
	var timeout=1500000;//localstroing的过期时间1500s
	for(var i=0;i<storage.length;i++){
		//key(i)获得相应的键，再用getItem()方法获得对应的值
		var keyjson=JSON.parse(storage.getItem(storage.key(i)));
		if(keyjson.time<(timenow-timeout)){
			localStorage.removeItem(storage.key(i));//清除过期的key
		}
	}
}
...
function videolist(){
	...
	if(token){
		...
		$.ajax({
			url:url,
			data:{'token':token,'video_id':<?php echo $video['videotab_id']; ?>,'video_type':<?php echo $video['video_type']; ?>},
			type:'post',
			datatype:'json',
			success:function(event){
				// alert(event.data);
